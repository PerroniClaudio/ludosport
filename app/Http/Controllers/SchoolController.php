...
class SchoolController extends Controller {
    ...
    public function getByAcademy(Request $request) {
        ...
        foreach ($schools as $key => $school) {
            ...
            if (in_array($authRole, ['dean']) && $school->id !== $primarySchool->id) {
                continue;
            }
            $formatted_schools[] = [
                'id' => $school->id,
    ...
    public function athletesDataForSchool(School $school) {
        $data = \Illuminate\Support\Facades\Cache::remember("athletes-school-{$school->id}-data", now()->addDay(), function () use ($school) {
            $athletes = $school->athletes()->withCount('clans')->get();
            $active_users = 0;
    ...
    public function getAthletesNumberPerYear(School $school) {
        $data = \Illuminate\Support\Facades\Cache::remember("athletes-school-{$school->id}-data-per-year", now()->addDay(), function () use ($school) {
