        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white dark:bg-background-800 overflow-hidden shadow-sm sm:rounded-lg p-8">
                    ...
                    <div class="w-1/2 flex flex-col gap-2">
                        ...
                        @if ($user->is_user_minor)
                            <div
                                class="mt-1 text-sm text-background-600 dark:text-background-200 flex flex-row items-center gap-2">
                                <x-lucide-baby class="w-6 h-6 text-amber-500" />
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @if ($canManageMinorDocuments)
                <x-modal name="upload-minor-document-{{ $user->id }}" :show="false" focusable>
                    <form method="POST"
                        action="{{ route('rector.users.approval-document.store', $user->id) }}"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4" x-data="{
                ...
                <x-modal name="set-main-institution-modal" :show="$errors->get('name') || $errors->get('go_to_edit')" focusable>
                    <form method="POST"
                        ...
                        {{-- Il rettore non può modificare l'accademia principale --}}
                        <template x-if="institutionType == 'academy' && roleType == 'personnel'">
                            @php
                                ...
                                // Per ora il rettore non può modificare l'accademia principale, quindi se servirà in futuro vedremo come fare.
                                $selectedAcademy = [
