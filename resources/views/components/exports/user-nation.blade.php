<form action="{{ route('exports.store') }}" method="POST" x-data="{
    ...
    availableNations: [],
    paginatedNations: [],
    paginatedselectedNations: [],
    currentPage: 1,
    totalPages: 1,
    usersType: '',
    isSubmitEnabled: false,
    getavailableNations: function() {
        fetch('/nations/all')
            .then(response => response.json())
            .then(data => {
                this.availableNations = data;
                this.paginatedNations = this.availableNations.slice(0, 10);
                this.totalPages = Math.ceil(this.availableNations.length / 10);
            })
            .catch(error => {
                console.error(error);
            });
    },
    searchavailableNations: function(event) {
        if (event.target.value.length >= 3) {
            fetch('/nations/search?search=' + event.target.value)
                .then(response => response.json())
                .then(data => {
                    this.availableNations = data;
                    this.paginatedNations = this.availableNations.slice(0, 10);
            ...
            this.getavailableNations();
        }
    },
    searchselectedNations: function(event) {
        if (event.target.value.length >= 3) {
            this.paginatedselectedNations = this.selectedNations.filter(nation => nation.name.toLowerCase().includes(event.target.value.toLowerCase()));
        } else {
            this.paginatedselectedNations = this.selectedNations;
        }
    },
    addNation: function(id) {
        if (this.selectedNations.find(nation => nation.id === id)) {
            return;
        }
        let nation = this.availableNations.find(nation => nation.id === id);
        this.selectedNations.push(nation);
        this.paginatedselectedNations = this.selectedNations;
        this.updateFilterJson()
    },
    removeNation: function(id) {
        this.selectedNations = this.selectedNations.filter(nation => nation.id !== id);
        this.paginatedselectedNations = this.selectedNations;
        this.updateFilterJson()
    },
    goToPage: function(page) {
        ...
        this.paginatedNations = this.availableNations.slice((page - 1) * 10, page * 10);
    },
    updateFilterJson: function() {
        this.filtersJson = JSON.stringify(this.selectedNations);
        this.validateForm()
    },
    validateForm: function() {
        if ((this.selectedNations.length > 0) && (this.usersType !== '')) {
            this.isSubmitEnabled = true;
            return;
        }
        this.isSubmitEnabled = false;
    },
    isNationSelected: function(id) {
        return this.selectedNations.find(nation => nation.id === id) !== undefined;
    },
    selectAllNations: function() {
        this.availableNations.forEach(nation => {
            if (!this.isNationSelected(nation.id)) {
                this.selectedNations.push(nation);
            }
        });
        this.paginatedselectedNations = this.selectedNations;
    ...
    deselectAllNations: function() {
        this.selectedNations = [];
        this.paginatedselectedNations = [];
        this.updateFilterJson();
    },
    init: function() {
        this.getavailableNations();
    ...
    <input name="type" type="hidden" value="users_nation">
    ...
    <div class="grid grid-cols-2 gap-2">
        <div class="bg-background-100 dark:bg-background-900 p-4 rounded">
            <div class="flex justify-between gap-2 items-center">
                ...
                <div>
                    <x-text-input type="text" x-on:input="searchavailableNations(event);" placeholder="Search..."
            ...
            <table
                ...
                <thead>
                    <tr>
                        ...
                        <th class="text-right bg-background-100 dark:bg-background-900 sticky top-0 border-b border-background-100 dark:border-background-700 py-2 text-primary-500 dark:text-primary-400 font-bold tracking-wider uppercase text-xs truncate">
                            <button type="button" @click="selectAllNations()"
                ...
                <tbody>
                    <template x-for="(row, index) in paginatedNations">
                        <tr :class="{ 'bg-primary-100 dark:bg-primary-900': isNationSelected(row.id) }">
                            ...
                            <td class="text-background-500 dark:text-background-300 text-sm text-right p-1">
                                <button type="button" @click="addNation(row.id)" x-bind:disabled="isNationSelected(row.id)"
                                    :class="{ 'opacity-40 cursor-not-allowed': isNationSelected(row.id) }">
                                    <x-lucide-plus
                                        class="w-4 h-4 text-primary-500 dark:text-primary-400 hover:text-primary-700" />
                                </button>
                                <button type="button" class="ml-2" x-show="isNationSelected(row.id)" @click="removeNation(row.id)">
                                    <x-lucide-minus
                                        class="w-4 h-4 text-primary-500 dark:text-primary-400 hover:text-primary-700" />
                                </button>
                            </td>
        ...
        <div class="bg-background-100 dark:bg-background-900 p-4 rounded">
            <div class="flex justify-between gap-2 items-center">
                ...
                <div>
                    <x-text-input type="text" x-on:input="searchselectedNations(event);" placeholder="Search..."
                        class="border border-background-100 dark:border-background-700 text-background-500 dark:text-background-300 rounded-lg p-2" />
                </div>
            </div>
            <div class="border-b border-background-100 dark:border-background-700 my-2"></div>
            ...
            <table
                ...
                <thead>
                    <tr>
                        ...
                        <th class="text-right bg-background-100 dark:bg-background-900 sticky top-0 border-b border-background-100 dark:border-background-700 py-2 text-primary-500 dark:text-primary-400 font-bold tracking-wider uppercase text-xs truncate">
                            <button type="button" @click="deselectAllNations()"
                                class="px-3 py-1 bg-primary-500 hover:bg-primary-600 dark:bg-primary-400 dark:hover:bg-primary-500 text-white rounded text-xs font-semibold transition-colors">
                ...
                <tbody>
                    <template x-for="(row, index) in paginatedselectedNations">
                        <tr>
                            <td class="text-background-500 dark:text-background-300 text-sm" x-text="row.name"></td>
                            <td class="text-background-500 dark:text-background-300 text-sm text-right p-1">
                                <button type="button" @click="removeNation(row.id)">
                                    <x-lucide-minus
                                        class="w-4 h-4 text-primary-500 dark:text-primary-400 hover:text-primary-700" />
                                </button>
                            </td>
