<form action="{{ route('technician.exports.store') }}" method="POST" x-data="{
    filters: [],
    filtersJson: '[]',
    selectedAcademies: [],
    availableAcademies: [],
    ...
    getavailableAcademies: function() {
        fetch('/technician/academies/all')
    ...
    searchavailableAcademies: function(event) {
        ...
        if (event.target.value.length >= 3) {
            fetch('/technician/academies/search?search=' + event.target.value)
                .then(response => response.json())
                .then(data => {
                    this.availableAcademies = data;
                    this.paginatedAcademies = this.availableAcademies.slice(0, 10);
    ...
    selectAllAcademies: function() {
        this.availableAcademies.forEach(academy => {
            if (!this.selectedAcademies.find(selectedAcademy => selectedAcademy.id === academy.id)) {
