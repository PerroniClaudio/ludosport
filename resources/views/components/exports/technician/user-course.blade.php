<form action="{{ route('technician.exports.store') }}" method="POST" x-data="{
    ...
    searchAvailableCourses: function(event) {
        ...
        if (event.target.value.length >= 3) {
            fetch('/technician/courses/search?search=' + event.target.value)
                .then(response => response.json())
                .then(data => {
                    this.availableCourses = data;
                    this.paginatedCourses = this.availableCourses.slice(0, 10);
    ...
    selectAllCourses: function() {
        this.availableCourses.forEach(course => {
            if (!this.selectedCourses.find(selectedCourse => selectedCourse.id === course.id)) {
                this.selectedCourses.push(course);
            }
        });
        this.paginatedSelectedCourses = this.selectedCourses;
