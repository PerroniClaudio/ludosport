...
<div class="flex flex-col gap-4">
    <div class="bg-white dark:bg-background-800 overflow-hidden shadow-sm sm:rounded-lg p-8">
        <h3 class="text-background-800 dark:text-background-200 text-2xl">{{ __('users.disable_user') }}</h3>
        <div class="border-b border-background-100 dark:border-background-700 my-2"></div>
        <div class="w-1/2 flex flex-col gap-2">
            <p class="mt-1 text-sm text-background-600 dark:text-background-200">
                {{ __('users.disable_user_message') }}
            </p>

            <div class="w-1/2">
                <x-danger-button 
                    x-data=""
                    x-on:click.prevent="$dispatch('open-modal', 'confirm-user-disable')">{{ __('users.disable_user') }}
                </x-danger-button>
            </div>
