...
<div x-data="{ selectedUserId: null }">
    ...
    <x-modal name="personnel-modal" :show="$errors->userId->isNotEmpty()" focusable>
        <form method="post" action="{{ route($addToRoute . 'academies.personnel.store', $academy->id) }}" class="p-6"
            x-ref="addform">
            @csrf
        ...
        <form method="post" action="{{ route($addToRoute . 'academies.personnel.remove', $academy->id) }}"
            class="p-6" x-ref="removeform">
            @csrf
