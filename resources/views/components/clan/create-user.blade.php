...
<div x-data="{
    ...
    <x-primary-button x-on:click.prevent="$dispatch('open-modal', 'new-user-{{ $type }}-modal')">
        @if ($type === 'personnel')
            <span>{{ __('clan.create_personnel') }}</span>
        @else
            <span>{{ __('clan.create_athlete') }}</span>
        @endif
    </x-primary-button>
    ...
    <x-modal name="new-user-{{ $type }}-modal" :show="$should_show_modal_for_errors" focusable>
        ...
        @endphp
        <form action="{{ route($editUserRoute, $clan) }}" method="post" enctype="multipart/form-data" class="p-6 flex flex-col gap-4">

            @csrf

            <input type="hidden" name="clan_id" value="{{ $clan }}">

            <div class="flex items-center justify-between">
                <h2 class="text-lg font-medium text-background-900 dark:text-background-100">
                    @if ($type === 'personnel')
                        {{ __('clan.quick_create_personnel') }}
                    @else
                        {{ __('clan.quick_create_athlete') }}
                    @endif
                </h2>
                <div>
            ...
            <x-form.checkbox name="go_to_edit" id="go_to_edit_{{ $type }}"
                label="{{ __('clan.go_to_edit') }}" />

            <div class="flex justify-end">
                <x-primary-button>
                    <span>
                        @if ($type === 'personnel')
                            {{ __('clan.create_personnel') }}
                        @else
                            {{ __('clan.create_athlete') }}
                        @endif
                    </span>
                </x-primary-button>
            </div>
